...
//    view->page()->profile()->cookieStore()->loadAllCookies();
//    view->page()->profile()->cookieStore()->sa

    profile = new QWebEngineProfile("pcm-life");
...
void WebController::setUrl(const QUrl &url)
{
    ...
    QString name = url.path().split("/").join("_");
    qDebug()<<"Profile : "<<name;

    CustomWebEnginePage *page = new CustomWebEnginePage(profile);
    ...
    page->setBackgroundColor(QColor(50,60,50));
